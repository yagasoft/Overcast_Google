...
package com.yagasoft.overcast.implement.google.container;
...
public class RemoteFolder extends com.yagasoft.overcast.base.container.remote.RemoteFolder<File>
{
	...
	{
		Logger.info("GOOGLE: FOLDER: checking existence: " + path);
		...
		{
			Logger.error("GOOGLE: FOLDER: failed to determine if folder exists or not: " + path);
			Logger.except(e);
			e.printStackTrace();
			...
			throws OperationException
	{
		try
		{
			List request = Google.driveService.files().list().setQ("trashed = false and '" + id + "' in parents");
			...
			do
			{
				try
				{
					...
					childrenResult.getItems().stream()
							.forEach(child ->
							{
			...
			if ( !childrenIds.isEmpty())
			{
				for (String id : childrenIds)
				{
					...
					String type = remote.getMimeType();

					if ((type != null) && (type.indexOf("folder") >= 0))
					{
	...
	{
		Logger.info("GOOGLE: FOLDER: UPDATING info from source: " + path);
		...
		try
		{
			...
			Logger.info("GOOGLE: FOLDER: FINISHED updating info from source: " + path);
		}
		catch (IOException e)
		{
			Logger.error("GOOGLE: FOLDER: FAILED to update info from source: " + path);
			Logger.except(e);
			e.printStackTrace();
	...
	public void deleteProcess() throws OperationException
	{
		try
		{
			Google.driveService.children().delete(getParent().getId(), id).execute();
